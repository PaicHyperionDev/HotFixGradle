...
class Processor {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (shouldProcessClassInJar(entryName, includePackage, excludeClass)) {
                    ...
                    if (MapUtils.notSame(map, entryName, hash)) {
                        FilePropUtils.copyBytesToFile(bytes, FilePropUtils.touchFile(patchDir, entryName))
                    }
                } else {
    ...
    static boolean shouldProcessClassInJar(String entryName, HashSet<String> includePackage, HashSet<String> excludeClass) {
        return entryName.endsWith(".class") && !entryName.startsWith("com/paic/hyperion/core/hotfix/") &&
                !entryName.startsWith("cn/jiajixin/nuwa") &&
                SetUtils.isIncluded(entryName, includePackage) && !SetUtils.isExcluded(entryName, excludeClass) && !entryName.contains("android/support/")
    }
